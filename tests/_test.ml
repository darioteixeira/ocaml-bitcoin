...
module Testnet = Bitcoin.Make (Bitcoin_cohttp_async.Httpclient) (Testnet_connection)
...
let test_getinfo global =
  match List.assoc "blocks" (Testnet.getinfo ()) with
...
let test_getblockcount global = assert_bool "block count must be > 0" (Testnet.getblockcount () > 0)
...
let test_backupwallet global = bracket_tmpfile (fun (fname, _) -> Testnet.backupwallet fname) ()
...
let test_dumpprivkey global =
  ...
  assert_raises
    (Bitcoin_error (-5, "Invalid private key encoding"))
    (fun () -> Testnet.importprivkey "");
  Testnet.importprivkey priv

let test_getaccount global = assert_equal global.account3 (Testnet.getaccount global.address3)
...
let test_listaddressgroupings global =
  assert_bool
    "number of address groupings must be >= 0"
    (List.length (Testnet.listaddressgroupings ()) >= 0)
...
let test_listunspent global =
  assert_bool
    "number of unspent transactions must be > 0"
    (List.length (Testnet.listunspent ~minconf:0 ()) > 0)
...
let test_sendfrom global =
  ...
  let default_balance' = Testnet.getbalance ~account:`Default () in
  assert_bool
    "Old balance must be >= new balance + amount"
    (default_balance >= Int64.add default_balance' amount)
...
let test_sendmany global =
  ...
  let default_balance' = Testnet.getbalance ~account:`Default () in
  assert_bool
    "Old balance must be >= new balance + amounts"
    (default_balance >= Int64.add default_balance' (Int64.add amount1 amount2))
...
let test_settxfee global =
  let gettxfee () =
    match List.assoc "paytxfee" (Testnet.getinfo ()) with
    | `Float x -> amount_of_float x
    | _ -> assert_failure "gettxfee"
  in
