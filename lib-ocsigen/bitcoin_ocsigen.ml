...
module Httpclient : Bitcoin.HTTPCLIENT with type 'a Monad.t = 'a Lwt.t = struct
  ...
  let post_string ~headers ~inet_addr ~host ~port ~uri request =
    ...
    let headers =
      ...
      List.fold_left adder empty headers
    in
    let%lwt frame =
    ...
    | Some stream ->
      ...
      let rec string_of_stream stream =
        match%lwt Ocsigen_stream.next stream with
        | Ocsigen_stream.Cont (str, stream) ->
      ...
      in
      let%lwt result = string_of_stream (Ocsigen_stream.get stream) in
      Ocsigen_stream.finalize stream `Success >> Lwt.return result
